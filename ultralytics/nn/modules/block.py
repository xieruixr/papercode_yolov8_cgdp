...
class DropPath(nn.Module):
    """Drop paths per sample, matching timm behavior without requiring timm."""
...
class Scale(nn.Module):
    """Learnable vector multiplier used by the CGDP MetaFormer block."""
...
class StarReLU(nn.Module):
    """StarReLU activation used by the ConvFormer token mixer."""
...
class LayerNormWithoutBias(nn.Module):
    """LayerNorm variant used by the compact CGDP MetaFormer-CGLU block."""
...
class SepConv(nn.Module):
    """Inverted separable convolution token mixer for ConvFormer-style blocks."""

    def __init__(self, dim, expansion_ratio=2, act1_layer=StarReLU, act2_layer=nn.Identity, bias=False, drop=0.0, **kwargs):
        super().__init__()
        med_channels = int(expansion_ratio * dim)
        self.pwconv1 = nn.Linear(dim, med_channels, bias=bias)
        self.act1 = act1_layer()
        self.dwconv = nn.Conv2d(med_channels, med_channels, kernel_size=7, padding=3, groups=med_channels, bias=bias)
...
class ConvolutionalGLU(nn.Module):
    """Convolutional gated linear unit used by C2f_ConvFormerCGLU."""
...
class MetaFormerCGLUBlock(nn.Module):
    """Minimal MetaFormer block required by the CGDP backbone."""
...
class C2f_ConvFormerCGLU(C2f):
    """C2f block with ConvFormer token mixing and convolutional GLU."""

    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(MetaFormerCGLUBlock(dim=self.c, token_mixer=SepConv) for _ in range(n))


class DySample(nn.Module):
    """Lightweight learned upsampler used inside DGAF."""
    ...
    def forward(self, x):
        if self.style == "pl":
            offset = F.pixel_shuffle(self.offset(F.pixel_unshuffle(x, self.scale)), self.scale)
...
class DGAF(nn.Module):
    ...
    def __init__(self, inc, input_dim=64, dysample_groups=2):
        ...
        if c % self.groups != 0:
            raise ValueError(f"C ({c}) must be divisible by groups ({self.groups}).")
        ...
        self.kernel_size = 3
        self.dcn_offset_mask = nn.Conv2d(c, self.groups * 3 * self.kernel_size * self.kernel_size, 3, stride=2, padding=1, groups=self.groups)
        self.dcn_conv = ModulatedDeformConv2d(c, c, 3, stride=2, padding=1, groups=self.groups, bias=False)
    ...
    def fuse_features(self, src, target):
        """Align src to target's spatial resolution."""
        if (src.shape[2] > target.shape[2]) or (src.shape[3] > target.shape[3]):
            ...
            offset_mask = self.dcn_offset_mask(src)
            offset, mask = torch.split(offset_mask, [offset_channels, mask_channels], dim=1)
            return self.dcn_conv(src, offset.contiguous(), mask.contiguous().sigmoid())
    ...
    def forward(self, x):
        ...
        low_gate = self.sigmoid(low_feature)
        high_gate = self.sigmoid(high_feature)
        low_feature = self.d_in1(low_feature)
        high_feature = self.d_in2(high_feature)

        low_fused = low_feature + low_feature * low_gate + (1 - low_gate) * self.fuse_features(high_gate * high_feature, low_feature)
        high_fused = high_feature + high_feature * high_gate + (1 - high_gate) * self.fuse_features(low_gate * low_feature, high_feature)
        return self.conv(torch.cat([self.fuse_features(high_fused, low_fused), low_fused], dim=1))


class DyDCNv2(nn.Module):
    """Modulated deformable convolution used by the DyHead block."""
    ...
    def forward(self, x, offset, mask):
        ...
        return self.norm(x) if self.with_norm else x


class DyReLU(nn.Module):
    """Dynamic ReLU used by DyHead."""

    def __init__(self, channels, reduction=4, lambda_a=1.0, K2=True, use_bias=True, init_a=(1.0, 0.0), init_b=(0.0, 0.0)):
        super().__init__()
        ...
        self.fc = nn.Sequential(
            ...
            nn.Linear(hidden, channels * 4 if K2 else channels * 2),
            nn.Hardsigmoid(inplace=True),
        )

    def forward(self, x):
        b, c, _, _ = x.shape
        theta = self.fc(self.avg_pool(x).view(b, c)).view(b, c, 4 if self.K2 else 2, 1, 1)
        if self.K2:
            a1 = (theta[:, :, 0] - 0.5) * self.lambda_a + self.init_a[0]
            b1 = theta[:, :, 1] - 0.5 + self.init_b[0] if self.use_bias else 0
            a2 = (theta[:, :, 2] - 0.5) * self.lambda_a + self.init_a[1]
            b2 = theta[:, :, 3] - 0.5 + self.init_b[1] if self.use_bias else 0
...
class DyHeadBlock(nn.Module):
    """Dynamic Head block used by Detect_DyHead."""
