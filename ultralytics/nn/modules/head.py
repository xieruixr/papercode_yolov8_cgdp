...
__all__ = "Detect", "Segment", "Pose", "Classify", "OBB", "RTDETRDecoder", "v10Detect", "Detect_DyHead", "Detect_LSCD"
...
class Detect_DyHead(nn.Module):
    """YOLOv8 detection head with Dynamic Head feature refinement.

    Args:
        nc (int): Number of classes.
        hidc (int): Hidden channel width used before DyHead blocks.
        block_num (int): Number of DyHead blocks.
        ch (tuple[int]): Input feature channels from the neck.
    """
    ...
    def forward(self, x):
        """Return raw training outputs or decoded inference predictions."""
        for i in range(self.nl):
            x[i] = self.conv[i](x[i])
        x = self.dyhead(x)
        shape = x[0].shape  # BCHW
