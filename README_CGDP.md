...
- `RGFM` in `ultralytics/nn/modules/block.py`
- `SPECA` in `ultralytics/nn/modules/SPECA.py`
...
```bash
yolo detect train model=config_model_yaml/yolov8_DCF-C2F_RGFM_SPECA_p234_dyhead.yaml data=dataset_yaml/VisDrone.yaml epochs=200 imgsz=640 batch=16 optimizer=SGD
```
...
Weights, datasets, training runs, result folders, videos, and caches are intentionally excluded from this code release.
